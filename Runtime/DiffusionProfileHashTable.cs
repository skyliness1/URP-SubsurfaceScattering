...
{
    /// <summary>
    /// This class keeps track of every diffusion profile in the project so it can generate unique uint hashes
    /// for every asset, which are used to differentiate diffusion profiles in the shader
    /// (Based on HDRP implementation)
    /// </summary>
    [InitializeOnLoad]
    internal class DiffusionProfileHashTable
    {
        ...
        public static void UpdateDiffusionProfileHashNow(DiffusionProfileSettings profile)
        {
            ...
            // Save assets if hash was modified
            if (needsSave)
            {

...
namespace SoulRender
{
    ...
    /// </summary>
    public static class DiffusionProfileConstants
    {
    ...
    public class DiffusionProfile : IEquatable<DiffusionProfile>
    {
        ...
        [Tooltip("Specifies when URP applies the albedo of the Material.")]
        ...
        public DiffusionProfile(bool initialize = true)
        {
            if (initialize)
            {
                ResetToDefault();
        ...
        public void Validate()
        {
            ...
            // Check if this is a newly created asset that needs proper initialization
            if (worldScale <= 0.0f)
            {
                // This is likely a new asset, reset to proper defaults
                ResetToDefault();
                return; // ResetToDefault() calls UpdateKernel(), so we can return early
            }
    ...
    /// </summary>
    [CreateAssetMenu(fileName = "New Diffusion Profile", menuName = "SubsurfaceScattering/Diffusion Profile Settings")]
    public class DiffusionProfileSettings : ScriptableObject
    {
        [SerializeField]
        public DiffusionProfile profile;

        // Cached vector data for passing to shaders (public for editor access)
        [NonSerialized] public Vector4 worldScaleAndFilterRadiusAndThicknessRemap; // X = meters per world unit, Y = filter radius (in mm), Z = remap start, W = end - start
        [NonSerialized] public Vector4 shapeParamAndMaxScatterDist;                // RGB = S = 1 / D, A = d = RgbMax(D)
        [NonSerialized] public Vector4 transmissionTintAndFresnel0;                // RGB = color, A = fresnel0
        [NonSerialized] public Vector4 disabledTransmissionTintAndFresnel0;        // RGB = black, A = fresnel0 - For debug to remove the transmission
        [NonSerialized] public Vector4 dualLobeAndDiffusePower;                    // R = Smoothness A, G = Smoothness B, B = Lobe Mix, A = Diffuse Power - 1 (to have 0 as neutral value)
        [NonSerialized] public Vector4 borderAttenuationColorMultiplier;           // RGB = color, A = not used
        ...
        public float primarySmoothnessMultiplier
        {
            ...
            set { profile.smoothnessMultipliers.y = Mathf.Clamp(value, 1, 2);
                  UpdateCache();
            }
        }
        ...
        public float secondarySmoothnessMultiplier
        {
            ...
            set { profile.smoothnessMultipliers.x = Mathf.Clamp(value, 0, 1);
                  UpdateCache();
            }
        }
        ...
        {
            get => profile.lobeMix;
            set { profile.lobeMix = value;
                  UpdateCache();
            }
        }

        /// <summary>
        /// Exponent on the cosine component of the diffuse lobe. Helps to simulate non lambertian surfaces.
        /// </summary>
        public float diffuseShadingPower
        {
            get => profile.diffuseShadingPower;
            set { profile.diffuseShadingPower = value;
                  UpdateCache();
            }
        }
        ...
        public Color transmissionTint
        {
            ...
            set { profile.transmissionTint = value;
                  profile.Validate();
                  UpdateCache();
            }
        }
        ...
        public Color borderAttenuationColor
        {
            ...
            set { profile.borderAttenuationColor = value;
                  profile.Validate();
                  UpdateCache();
            }
        }
        ...
        internal void SetDefaultParams()
        {
            ...
            transmissionTintAndFresnel0.w = 0.04f; // Match DEFAULT_SPECULAR_VALUE
        }

        /// <summary>
        /// Convert HDR color to LDR color and multiplier
        /// </summary>
        private static void ConvertHDRColorToLDR(Color hdrColor, out Color ldrColor, out float multiplier)
        {
            float maxComponent = Mathf.Max(Mathf.Max(hdrColor.r, hdrColor.g), hdrColor.b);
            
            if (maxComponent > 1.0f)
            {
                multiplier = maxComponent;
                ldrColor = new Color(hdrColor.r / multiplier, hdrColor.g / multiplier, hdrColor.b / multiplier, hdrColor.a);
            }
            else
            {
                multiplier = 1.0f;

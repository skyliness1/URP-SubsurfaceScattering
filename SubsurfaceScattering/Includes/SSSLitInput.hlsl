    ...
    half3 normalTS)
{
    ...
    float2 deltaUV = offsetScale.y * viewDirTS.xy * rcp(safeDenominator * slicesNum);
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        halfDeltaUV *= 0.5;
        halfDeltaHeight *= 0.5;
        half delta = currHeight - rayHeight;
        if (abs(delta) <= 0.01)
            break;
        if(delta > 0)
        {
    ...
    half layer4Mask)          // 这是原始的 Layer4 mask 值
{
    ...
    // Layer 1: 如果勾选了 Layer1，使用 layer1Weight
    if (layerMask.x > 0.5)
    {
        totalWeight += layer1Weight;
    }
    
    // Layer 2: 如果勾选了 Layer2，使用原始的 layer2Mask
    if (layerMask.y > 0.5)
    {
        totalWeight += layer2Mask;
    }
    
    // Layer 3: 如果勾选了 Layer3，使用原始的 layer3Mask
    if (layerMask.z > 0.5)
    {
        totalWeight += layer3Mask;
    }
    
    // Layer 4: 如果勾选了 Layer4，使用原始的 layer4Mask
    if (layerMask.w > 0.5)
    {
        totalWeight += layer4Mask;
    }
...
{
    // 计算该通道在当前像素的总权重
    // 根据各层的 mask 和该通道的 layerMask 设置来决定
    half totalWeight = 0;
    
    // Layer 1 权重 = 1 - 其他层的覆盖
    half effectiveLayer1Weight = layer1Weight;
    if (layerMask.x > 0.5)
        totalWeight += effectiveLayer1Weight;

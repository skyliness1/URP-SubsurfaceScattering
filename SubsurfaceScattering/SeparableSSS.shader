Shader "Hidden/SubsurfaceScattering/SeparableSSS"
{
    ...
    float4 _TempDepthAttachment_TexelSize;
    TEXTURE2D_X(_MainTex);
    TEXTURE2D_X(_TempDepthAttachment);
    SAMPLER(sampler_TempDepthAttachment);
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_TARGET
            {
                ...
                float dSceneDepth = SAMPLE_TEXTURE2D_X(_TempDepthAttachment, sampler_TempDepthAttachment, texcoord).r;
                ...
                float2 finalStep = _SSSSDirection.xy * scale * _TempDepthAttachment_TexelSize.xy * subsurfaceMask;
                ...
                for (int i = 1; i < nSamples; i++)
                {
                    ...
                    float depth = LinearEyeDepth(SAMPLE_TEXTURE2D_X(_TempDepthAttachment, sampler_TempDepthAttachment, offset), _ZBufferParams);
                ...
                if (abs(_SSSSDirection.x) < 0.0001)
                {
                    ...
                    float3 scatterLuminance = Luminance(postScatterAlbedo);
                    postScatterAlbedo = lerp(scatterLuminance, postScatterAlbedo, 0.65);
                    colorBlurred.rgb *= postScatterAlbedo * 1.15;
                }
